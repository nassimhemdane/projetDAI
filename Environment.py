...
class Environment:
    ...
    def __init__(self, tailleX, tailleY, posUnload):
        ...
        self.logs = [] # Logs for visualization purpuse
    ...
    def move(self, agent:MyAgent, x1, y1, x2, y2):
        ...
        if ( not self.isAt(agent, x1, y1)) or self.grilleAgent[x2][y2] != None  : # position already occupied
            print("position not free",self.grilleAgent[x2][y2].state)
            return False
        else :
    ...
    def load(self, agent ):
        ...
        if(self.grilleTres[x][y] != None  and self.grilleTres[x][y].getType() == agent.getType() ) :
            ...
            print("load fail",self.grilleTres[x][y] ,self.grilleTres[x][y].getType(),agent.getType(),agent.id,agent.getPos())
