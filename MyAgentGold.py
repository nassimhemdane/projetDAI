...
class MyAgentGold(MyAgent):
    ...
    def setup(self, auctioneer=None):
        ...
        if (not self.auctioneer):
            self.machine = Machine(model=self, states=MyAgentGold.states, initial="Init")
            ...
            self.machine = Machine(model=self, states=MyAgentGold.states2, initial="Init")
            self.machine.add_transition("NegStep", "BeginPlan", "BeginPlan")
    ...
    def create_bid_list(self):
        ...
        goldagents = self.getgoldagents(self.env)
        self.players = []
        for k in goldagents:
            # print(k.id)
            if (k.id != self.id):
    ...
    def create_my_plan(self, tour):
        ...
        for jj in placesf:
            ...
            it = it + 1
        isopening = "unloading:" + str(lp) + str(random.random() + it)
        while(isopening in self.states):
            isopening = "unloading:" + str(lp) + str(random.random() + it)
        ...
        if(len(self.chestlist)>0):
            ...
            for i in range(len(posvertex)):
                if (dist(pos, posvertex[i]) < min):
                    min = dist(pos, posvertex[i])
                    imin = i
            print("constraints",self.constraints.values())

            redcont=list(self.constraints.values())
            for i in range(len(redcont)):
                redcont[i]=redcont[i]-(min-1)
            tour = tournewc(self.pathGraph, imin,redcont)
            if (len(tour) > 0):
                ###create the plan
                self.create_my_plan(tour)
                self.machine.add_transition("ExecStep", "BeginPlan", str((self.posX, self.posY)))
